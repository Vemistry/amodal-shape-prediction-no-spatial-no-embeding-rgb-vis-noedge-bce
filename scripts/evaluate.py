"""
===================================================================================
ĐÁNH GIÁ MÔ HÌNH AMODAL PREDICTION (Cập nhật Precision & Recall)
===================================================================================
...
def calculate_metrics(pred_logits, target, visible, threshold=0.5):
    ...
    # ─────────────────────────────────────────────────────────────
    # Tính Intersection và Union chung
    # ─────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────
    # 3. Tính Precision & Recall (Pixel-level)
    # ─────────────────────────────────────────────────────────────
    # Precision = TP / (TP + FP) -> Giao của dự đoán và nhãn chia cho tổng dự đoán
    precision = (intersection + 1e-6) / (pred.sum(dim=(2, 3)) + 1e-6)
    
    # Recall = TP / (TP + FN) -> Giao của dự đoán và nhãn chia cho tổng nhãn thực tế
    recall = (intersection + 1e-6) / (target.sum(dim=(2, 3)) + 1e-6)

    # ─────────────────────────────────────────────────────────────
    # 4. Tính IoU cho vùng bị che khuất (Invisible IoU)
    # ─────────────────────────────────────────────────────────────
    # Vùng bị che khuất = amodal - visible
...
def evaluate(args):
    ...
    with torch.no_grad():
        for inputs, targets, occluded_region, _ in tqdm(
            ...
            visible_masks = inputs[:, 3:4, :, :].float().to(device) # Đồng bộ device

            outputs = model(inputs)
            
            # Unpack thêm precision và recall
...
def parse_args():
    ...
    parser.add_argument("--output", type=str, default="", help="Lưu kết quả ra file JSON")
